<?php

class ES_Sync_Statii {
	private $single_site_status = array(
		'posts_processed' => 0,
		'start_time' => 0,
	);

	/**
	 * Do nothing
	 *
	 * @since 0.1.0
	 */
	public function __construct() { }


	/**
	 * Get current sync status for a specific site or the global setup. Site syncs statii are
	 * stored in the option under their site id. The global cross-site index status is stored
	 * in the 0th key of the option. We use get_site_option since it will default to get_option
	 * in the event where multi-site is not setup. All syncs are stored in one option.
	 *
	 * null => current site id
	 * 0 => N/A
	 * 1 >= specific site
	 *
	 * @param int $site_id
	 * @since 0.1.0
	 * @return array
	 */
	public function get_status( $site_id = null ) {
		$option = get_site_option( 'es_status_by_site', array() );

		if ( empty( $site_id ) ) {
			$site_id = get_current_blog_id();
		}

		if ( isset( $option[$site_id] ) ) {
			return $option[$site_id];
		}

		return $this->single_site_status;
	}

	/**
	 * Update sync for a given site or the global setup ($site_id = 0). We use
	 * update_site_option since it will default to update_option when multi-site
	 * is not setup.
	 *
	 * @param array|int|string|object $status
	 * @param int $site_id
	 * @return bool
	 */
	public function update_status( $status, $site_id = null ) {
		$option = get_site_option( 'es_status_by_site', array() );

		if ( empty( $site_id ) ) {
			$site_id = get_current_blog_id();
		}

		$option[$site_id] = $status;

		return update_site_option( 'es_status_by_site', $option );
	}

	/**
	 * Check if a sync is in progress for a given site or globally ($site_id = 0)
	 *
	 * @param int $site_id
	 * @since 0.1.0
	 * @return bool
	 */
	public function is_sync_alive( $site_id = null ) {
		if ( empty( $site_id ) ) {
			$site_id = get_current_blog_id();
		}

		$sync_status = $this->get_status( $site_id );

		return ( ! empty( $sync_status['start_time'] ) );
	}

	/**
	 * Return how many syncs are alive cross-network
	 *
	 * @since 0.1.0
	 * @return int
	 */
	public function get_alive_sync_count() {
		$sites = wp_get_sites();
		$alive_syncs = 0;

		foreach ( $sites as $site ) {
			if ( $this->is_sync_alive( $site['blog_id'] ) ) {
				$alive_syncs++;
			}
		}

		return $alive_syncs;
	}

	/**
	 * Reset a sync for a specific site or globally
	 *
	 * @param int $site_id
	 * @since 0.1.0
	 */
	public function reset_sync( $site_id = null ) {
		if ( empty( $site_id ) ) {
			$site_id = get_current_blog_id();
		}

		$this->update_status( $this->single_site_status, $site_id );
	}

	/**
	 * Return a singleton instance of the current class
	 *
	 * @since 0.1.0
	 * @return ES_Sync_Statii
	 */
	public static function factory() {
		static $instance = false;

		if ( ! $instance ) {
			$instance = new self();
		}

		return $instance;
	}
}

global $es_sync_statii;
$es_sync_statii = ES_Sync_Statii::factory();

/**
 * Accessor functions for methods in above class. See doc blocks above for function details.
 */

function es_get_sync_status( $site_id = null ) {
	global $es_sync_statii;

	return $es_sync_statii->get_status( $site_id );
}

function es_update_sync_status( $status, $site_id = null ) {
	global $es_sync_statii;

	return $es_sync_statii->update_status( $status, $site_id );
}

function es_get_alive_sync_count() {
	global $es_sync_statii;

	return $es_sync_statii->get_alive_sync_count();
}

function es_reset_sync( $site_id = null ) {
	global $es_sync_statii;

	return $es_sync_statii->reset_sync( $site_id );
}

function es_is_sync_alive( $site_id = null ) {
	global $es_sync_statii;

	return $es_sync_statii->is_sync_alive( $site_id );
}