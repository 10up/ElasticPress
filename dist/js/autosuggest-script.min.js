...
/* harmony import */ var _utils_helpers__WEBPACK_IMPORTED_MODULE_26__ = __webpack_require__(/*! ./utils/helpers */ "./assets/js/utils/helpers.js");
/* harmony import */ var element_closest__WEBPACK_IMPORTED_MODULE_27__ = __webpack_require__(/*! element-closest */ "./node_modules/element-closest/index.mjs");
/* harmony import */ var promise_polyfill_src_polyfill__WEBPACK_IMPORTED_MODULE_28__ = __webpack_require__(/*! promise-polyfill/src/polyfill */ "./node_modules/promise-polyfill/src/polyfill.js");
/* harmony import */ var whatwg_fetch__WEBPACK_IMPORTED_MODULE_29__ = __webpack_require__(/*! whatwg-fetch */ "./node_modules/whatwg-fetch/fetch.js");
...
var _window = window,
    epas = _window.epas; // Ensure we have an endpoint URL, or
// else this shouldn't happen

if (epas.endpointUrl && epas.endpointUrl !== '') {
  init(); // Publically expose API
...
function triggerAutosuggestEvent(detail) {
  var event = new CustomEvent('ep-autosuggest-click', {
    detail: detail
  });
  window.dispatchEvent(event);

  if (detail.searchTerm && parseInt(epas.triggerAnalytics, 10) === 1 && typeof gtag === 'function') {
    var action = "click - ".concat(detail.searchTerm); // eslint-disable-next-line no-undef
...
function getJsonQuery() {
  if (typeof window.epas === 'undefined') {
    var error = 'No epas object defined'; // eslint-disable-next-line no-console
...
function buildSearchQuery(searchText, placeholder, _ref) {
  var query = _ref.query;
  var newQuery = Object(_utils_helpers__WEBPACK_IMPORTED_MODULE_26__["replaceGlobally"])(query, placeholder, searchText);
  return newQuery;
}
/**
...
function esSearch(_x, _x2) {
  return _esSearch.apply(this, arguments);
}
/**
...
function _esSearch() {
  _esSearch = _babel_runtime_helpers_asyncToGenerator__WEBPACK_IMPORTED_MODULE_25___default()(
  /*#__PURE__*/
  _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_23___default.a.mark(function _callee2(query, searchTerm) {
    var fetchConfig, response, data;
    return _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_23___default.a.wrap(function _callee2$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            ...
            _context2.prev = 2;
            _context2.next = 5;
            return window.fetch(epas.endpointUrl, fetchConfig);
          ...
          case 10:
            ...
            return _context2.abrupt("return", window.epDataFilter(data, searchTerm));
...
function updateAutosuggestBox(options, input) {
  var i;
  var itemString = ''; // get the search term for use later on

  var value = input.value;
  var container = Object(_utils_helpers__WEBPACK_IMPORTED_MODULE_26__["findAncestorByClass"])(input, 'ep-autosuggest-container');
  var resultsContainer = container.querySelector('.ep-autosuggest');
  var suggestList = resultsContainer.querySelector('.autosuggest-list'); // empty the the list of all child nodes
  ...
  var resultsLimit = options.length; // create markup for list items
  ...
  var _loop = function _loop() {
    var _options$i = options[i],
        ...
        url = _options$i.url;
    var escapedText = Object(_utils_helpers__WEBPACK_IMPORTED_MODULE_26__["escapeDoubleQuotes"])(text);
    var searchParts = value.trim().split(' ');
    var resultsText = escapedText; // uses some regex magic to match upper/lower/capital case

    searchParts.forEach(function (word) {
      var regex = new RegExp("(".concat(word.trim(), ")"), 'gi');

      if (word.length > 1) {
        resultsText = resultsText.replace(regex, "<span class=\"ep-autosuggest-highlight\">$1</span>");
      }
    });
    itemString += "<li class=\"autosuggest-item\" role=\"option\" aria-selected=\"false\" id=\"autosuggest-option-".concat(i, "\">\n\t\t\t\t<a href=\"").concat(url, "\" class=\"autosuggest-link\" data-search=\"").concat(escapedText, "\" data-url=\"").concat(url, "\"  tabindex=\"-1\">\n\t\t\t\t\t").concat(resultsText, "\n\t\t\t\t</a>\n\t\t\t</li>");
  };
  ...
  var autosuggestItems = Array.from(document.querySelectorAll('.autosuggest-link'));
...
function hideAutosuggestBox() {
  var lists = document.querySelectorAll('.autosuggest-list');
  var containers = document.querySelectorAll('.ep-autosuggest'); // empty all EP results lists
...
function checkForOrderedPosts(hits, searchTerm) {
  ...
  var filteredHits = hits.filter(function (hit) {
    ...
    if (undefined !== hit._source.terms && undefined !== hit._source.terms[taxName]) {
      hit._source.terms[taxName].forEach(function (currentTerm) {
  ...
  var orderedInserts = {};
  Object.keys(toInsert).sort().forEach(function (key) {
    orderedInserts[key] = toInsert[key];
  });

  if (Object.keys(orderedInserts).length > 0) {
    Object.keys(orderedInserts).forEach(function (key) {
...
function init() {
  var epInputNodes = document.querySelectorAll(".ep-autosuggest, input[type=\"search\"], .search-field, ".concat(epas.selector)); // build the container into which we place the search results.
  // These will be cloned later for each instance
  // of autosuggest inputs

  var epAutosuggest = document.createElement('div');
  ...
  epAutosuggest.appendChild(autosuggestList); // Build the auto-suggest containers
  // excluding the facet search field

  var epInputs = Array.from(epInputNodes).filter(function (node) {
    return !node.classList.contains('facet-search');
  });
  epInputs.forEach(function (input) {
    var epContainer = document.createElement('div');
    epContainer.classList.add('ep-autosuggest-container'); // Disable autocomplete

    input.setAttribute('autocomplete', 'off'); // insert the container - later we will place
    // the input inside this container

    input.insertAdjacentElement('afterend', epContainer); // move the input inside the container

    var form = input.closest('form');
    var container = form.querySelector('.ep-autosuggest-container');
    container.appendChild(input);
    var clonedContainer = epAutosuggest.cloneNode(true);
    input.insertAdjacentElement('afterend', clonedContainer); // announce that this is has been done

    var event = new CustomEvent('elasticpress.input.moved');
    input.dispatchEvent(event);
  });

  if (epInputs.length > 0) {
    epAutosuggest.setAttribute('style', "\n\t\t\ttop: ".concat(epInputs[0].offsetHeight - 1, ";\n\t\t\tbackground-color: ").concat(getComputedStyle(epInputs[0], 'background-color'), "\n\t\t"));
  }
  /**
  ...
  var formatSearchResults = function formatSearchResults(hits) {
    return hits.map(function (hit) {
      var text = hit._source.post_title;
      var url = hit._source.permalink;
      return {
        text: text,
        url: url
      };
  ...
  var currentIndex; // these are the keycodes we listen for in handleUpDown,
  // and in handleKeyup
  ...
  var handleUpDown = function handleUpDown(event) {
    ...
    var container = Object(_utils_helpers__WEBPACK_IMPORTED_MODULE_26__["findAncestorByClass"])(input, 'ep-autosuggest-container');
    var suggestList = container.querySelector('.autosuggest-list');
    var results = suggestList.children;
    /**
    ...
    var getSelectedResultIndex = function getSelectedResultIndex() {
      var resultsArr = Array.from(results);
      return resultsArr.findIndex(function (result) {
    ...
    var deSelectResults = function deSelectResults() {
    ...
    var selectNextResult = function selectNextResult() {
      if (currentIndex >= 0) {
    ...
    switch (event.keyCode) {
      ...
      case 40:
        ...
        if (typeof currentIndex === 'undefined') {
          ...
          var current = getSelectedResultIndex(); // check for existence of next result
  ...
  /*#__PURE__*/
  function () {
    var _ref2 = _babel_runtime_helpers_asyncToGenerator__WEBPACK_IMPORTED_MODULE_25___default()(
    /*#__PURE__*/
    _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_23___default.a.mark(function _callee(input) {
      var searchText, placeholder, queryJSON, query, response, hits, formattedResults;
      return _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_23___default.a.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              searchText = input.value;
              placeholder = 'ep_autosuggest_placeholder'; // retrieves the PHP-genereated query to pass to ElasticSearch

              queryJSON = getJsonQuery();
            ...
            case 5:
              ...
              query = buildSearchQuery(searchText, placeholder, queryJSON); // fetch the results

              _context.next = 9;
              return esSearch(query, searchText);

            case 9:
              response = _context.sent;

              if (response && response._shards && response._shards.successful > 0) {
                hits = checkForOrderedPosts(response.hits.hits, searchText);
                formattedResults = formatSearchResults(hits);
    ...
    return function fetchResults(_x3) {
  ...
  var handleKeyup = function handleKeyup(event) {
    ...
    var input = event.target;
    var debounceFetchResults = Object(_utils_helpers__WEBPACK_IMPORTED_MODULE_26__["debounce"])(fetchResults, 200);
  ...
  epInputs.forEach(function (input) {
    input.addEventListener('keyup', handleKeyup); // input.addEventListener( 'blur', hideAutosuggestBox );
  });
}
...
/*!************************************!*\
  !*** ./assets/js/utils/helpers.js ***!
  \************************************/
/*! exports provided: debounce, escapeRegExp, replaceGlobally, escapeDoubleQuotes, findAncestorByClass, pluck, formatPostBody, showElements, hideElements */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "replaceGlobally", function() { return replaceGlobally; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "escapeDoubleQuotes", function() { return escapeDoubleQuotes; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "findAncestorByClass", function() { return findAncestorByClass; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "pluck", function() { return pluck; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "formatPostBody", function() { return formatPostBody; });
...
var debounce = function debounce(fn, delay) {
  var timer = null; // don't use a fat arrow in order to preserve the proper context
...
var replaceGlobally = function replaceGlobally(string, term, replacement) {
...
var escapeDoubleQuotes = function escapeDoubleQuotes(str) {
...
var findAncestorByClass = function findAncestorByClass(el, className) {
  // eslint-disable-next-line
...
var formatPostBody = function formatPostBody(obj) {
...
var showOrHideNodes = function showOrHideNodes(els, display) {
  var nodes = []; // convert nodelist to array
  ...
  if (!nodes.length) {
    if (Array.isArray(els)) {
      nodes = [].concat(_babel_runtime_helpers_toConsumableArray__WEBPACK_IMPORTED_MODULE_12___default()(els), _babel_runtime_helpers_toConsumableArray__WEBPACK_IMPORTED_MODULE_12___default()(nodes));
    } else {
      nodes.push(els);
    }
  }
...
var showElements = function showElements(els) {
  return showOrHideNodes(els, 'inline-block');
};
/**
...
var hideElements = function hideElements(els) {
  return showOrHideNodes(els, 'none');
};
