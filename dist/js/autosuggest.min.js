...
var _typeof = typeof Symbol === "function" && typeof Symbol.iterator === "symbol" ? function (obj) { return typeof obj; } : function (obj) { return obj && typeof Symbol === "function" && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; }; /* eslint-disable camelcase */
...
function buildSearchQuery(searchText, postTypes, postStatus, searchFields) {
	...
	postTypes.map(function (postType) {
		var postTypeWeights = void 0,
		    postTypeWeightableFields = void 0;

		if (!_window.epas.weightableFields[postType]) {
			return;
		}

		var fields = [];

		postTypeWeights = _window.epas.weighting[postType] || {};
		postTypeWeightableFields = _window.epas.weightableFields[postType];
		...
		for (var i = 0; i < postTypeWeightableFields.length; i++) {
			var field = postTypeWeightableFields[i];
			var weight = postTypeWeights[field];
	...
	if (undefined !== _typeof(_window.epas.dateDecay) && true === _window.epas.dateDecay.enabled) {
		query.query = {
			function_score: {
				query: {
					bool: {
