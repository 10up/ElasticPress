...
class Elasticsearch {
	...
	/**
	 * Determine version of mapping currently on an index
	 *
	 * @param string $index Index name.
	 * @since 3.1.5
	 * @return string $version
	 */
	public function determine_mapping_version( $index ) {
		$path = apply_filters( 'ep_get_mapping_request_path', $index . '/_mapping' );
		...
		$request = $this->remote_request( $path, $request_args, [], 'determine_mapping_version' );
		...
		$mapping = json_decode( wp_remote_retrieve_body( $request ), true );
